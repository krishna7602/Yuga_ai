// ─────────────────────────────────────────────────────────────────────────────
// STUDY_DATA
//
// Each sentence now has a `phrases` array for phrase-by-phrase bilingual reading.
// Pattern: English phrase → Malayalam phrase → next English phrase → ...
// The full `english` / `malayalam` strings remain for display / word-highlight.
// ─────────────────────────────────────────────────────────────────────────────
export const STUDY_DATA = [
  {
    id: 1,
    title: "Paragraph 1",
    englishText: "The light was weak, even at noon. When Pelayo was coming back to the house, it was hard for him to see what was moving and groaning in the rear of the courtyard.",
    sentences: [
      {
        id: "s1-1",
        english: "The light was weak, even at noon.",
        malayalam: "വെളിച്ചം വളരെ കുറവായിരുന്നു, ഉച്ചസമയത്ത് പോലും.",
        phrases: [
          { english: "The light was weak",          malayalam: "വെളിച്ചം വളരെ കുറവായിരുന്നു" },
          { english: "even at noon.",               malayalam: "ഉച്ചസമയത്ത് പോലും." }
        ]
      },
      {
        id: "s1-2",
        english: "When Pelayo was coming back to the house, it was hard for him to see what was moving and groaning in the rear of the courtyard.",
        malayalam: "പെലായോ തിരികെ വീട്ടിലേക്ക് വരുമ്പോൾ, മുറ്റത്തിന്റെ പിന്നാമ്പുറത്ത് എന്തോ ഒന്ന് അനങ്ങുന്നതും മൂളുന്നതും വ്യക്തമായി കാണാൻ അവന് ബുദ്ധിമുട്ടായിരുന്നു.",
        phrases: [
          { english: "When Pelayo was coming back to the house,", malayalam: "പെലായോ തിരികെ വീട്ടിലേക്ക് വരുമ്പോൾ," },
          { english: "it was hard for him to see what was moving and groaning in the rear of the courtyard.", malayalam: "മുറ്റത്തിന്റെ പിന്നാമ്പുറത്ത് എന്തോ ഒന്ന് അനങ്ങുന്നതും മൂളുന്നതും വ്യക്തമായി കാണാൻ അവന് ബുദ്ധിമുട്ടായിരുന്നു." }
        ]
      }
    ],
    vocabulary: [
      { word: "Light was weak",  meaning: "വെളിച്ചം കുറവായിരുന്നു / മങ്ങിയിരിക്കുകയായിരുന്നു." },
      { word: "Even at noon",    meaning: "ഉച്ചസമയത്ത് പോലും (സാധാരണ ഉച്ചയ്ക്ക് നല്ല വെളിച്ചം ഉണ്ടാകേണ്ടതാണ്, എന്നാൽ ഇവിടെ പ്രകൃതി ഇരുണ്ടതായിരുന്നു)." },
      { word: "Rear",            meaning: "പിന്നാമ്പുറം." },
      { word: "Courtyard",       meaning: "മുറ്റം." },
      { word: "Moving",          meaning: "അനങ്ങുന്ന." },
      { word: "Groaning",        meaning: "വേദനകൊണ്ട് മൂളുന്ന." }
    ]
  },
  {
    id: 2,
    title: "Paragraph 2",
    englishText: "He had to go very close to see that it was an old man, a very old man. The man was lying face down in the mud. In spite of his tremendous efforts, the man couldn't get up, impeded by his enormous wings.",
    sentences: [
      {
        id: "s2-1",
        english: "He had to go very close to see that it was an old man, a very old man.",
        malayalam: "അതൊരു വൃദ്ധനാണെന്ന്, വളരെ പ്രായമായ ഒരു മനുഷ്യനാണെന്ന് തിരിച്ചറിയാൻ അവന് അയാളുടെ വളരെ അടുത്തേക്ക് പോകേണ്ടി വന്നു.",
        phrases: [
          { english: "He had to go very close",        malayalam: "അവന് അയാളുടെ വളരെ അടുത്തേക്ക് പോകേണ്ടി വന്നു" },
          { english: "to see that it was an old man,", malayalam: "അതൊരു വൃദ്ധനാണെന്ന് തിരിച്ചറിയാൻ," },
          { english: "a very old man.",                malayalam: "വളരെ പ്രായമായ ഒരു മനുഷ്യൻ." }
        ]
      },
      {
        id: "s2-2",
        english: "The man was lying face down in the mud.",
        malayalam: "ആ മനുഷ്യൻ ചെളിയിൽ കമിഴ്ന്നു വീണുകിടക്കുകയായിരുന്നു.",
        phrases: [
          { english: "The man was lying face down", malayalam: "ആ മനുഷ്യൻ കമിഴ്ന്നു വീണുകിടക്കുകയായിരുന്നു" },
          { english: "in the mud.",                 malayalam: "ചെളിയിൽ." }
        ]
      },
      {
        id: "s2-3",
        english: "In spite of his tremendous efforts, the man couldn't get up, impeded by his enormous wings.",
        malayalam: "തന്റെ പരമാവധി ശ്രമങ്ങൾ നടത്തിയിട്ടും ആ വൃദ്ധന് എഴുന്നേൽക്കാൻ കഴിഞ്ഞില്ല, അയാളുടെ ഭീമാകാരമായ വലിയ ചിറകുകൾ അതിന് തടസ്സമായി നിന്നു.",
        phrases: [
          { english: "In spite of his tremendous efforts,", malayalam: "തന്റെ പരമാവധി ശ്രമങ്ങൾ നടത്തിയിട്ടും," },
          { english: "the man couldn't get up,",            malayalam: "ആ വൃദ്ധന് എഴുന്നേൽക്കാൻ കഴിഞ്ഞില്ല," },
          { english: "impeded by his enormous wings.",      malayalam: "അയാളുടെ ഭീമാകാരമായ വലിയ ചിറകുകൾ അതിന് തടസ്സമായി നിന്നു." }
        ]
      }
    ],
    vocabulary: [
      { word: "Face down",         meaning: "മുഖം നിലത്തു കുത്തി / കമിഴ്ന്ന്." },
      { word: "Mud",               meaning: "ചെളി." },
      { word: "In spite of",       meaning: "ഉണ്ടായിരുന്നിട്ടും (ശ്രമിച്ചിട്ടും)." },
      { word: "Tremendous efforts",meaning: "കഠിനമായ ശ്രമങ്ങൾ / പരമാവധി ശ്രമം." },
      { word: "Impeded",           meaning: "തടസ്സപ്പെട്ടു (എഴുന്നേൽക്കാൻ കഴിയാത്തവിധം തടസ്സമായി മാറി)." },
      { word: "Enormous wings",    meaning: "ഭീമാകാരമായ ചിറകുകൾ." }
    ]
  },
  {
    id: 3,
    title: "Paragraph 3",
    englishText: "Frightened by the sight, Pelayo ran to get Elisenda, his wife. She was putting compresses on their sick child.",
    sentences: [
      {
        id: "s3-1",
        english: "Frightened by the sight, Pelayo ran to get Elisenda, his wife.",
        malayalam: "ആ കാഴ്ച കണ്ട് ഭയന്നുപോയ പെലായോ, തന്റെ ഭാര്യയായ എലിസെൻഡയെ വിളിക്കാനായി ഓടി.",
        phrases: [
          { english: "Frightened by the sight,",          malayalam: "ആ കാഴ്ച കണ്ട് ഭയന്നുപോയ," },
          { english: "Pelayo ran to get Elisenda,",       malayalam: "പെലായോ എലിസെൻഡയെ വിളിക്കാനായി ഓടി," },
          { english: "his wife.",                         malayalam: "തന്റെ ഭാര്യ." }
        ]
      },
      {
        id: "s3-2",
        english: "She was putting compresses on their sick child.",
        malayalam: "അവൾ അപ്പോൾ അവരുടെ സുഖമില്ലാത്ത കുഞ്ഞിന്റെ ശരീരത്തിൽ പനി മാറാൻ വേണ്ടി നനച്ച തുണി തുണ്ടുകൾ വെച്ചുകൊടുക്കുകയായിരുന്നു.",
        phrases: [
          { english: "She was putting compresses", malayalam: "അവൾ നനച്ച തുണി തുണ്ടുകൾ വെച്ചുകൊടുക്കുകയായിരുന്നു" },
          { english: "on their sick child.",       malayalam: "അവരുടെ സുഖമില്ലാത്ത കുഞ്ഞിന്റെ ശരീരത്തിൽ." }
        ]
      }
    ],
    vocabulary: [
      { word: "Frightened",        meaning: "ഭയന്നുപോയി / പേടിച്ചു." },
      { word: "Sight",             meaning: "കാഴ്ച." },
      { word: "Ran to get",        meaning: "വിളിക്കാൻ ഓടി / കൂട്ടിക്കൊണ്ടുവരാൻ ഓടി." },
      { word: "Putting compresses",meaning: "നനച്ച തുണി ശരീരത്തിൽ വെക്കുക (പനി മാറ്റാൻ തുണി നനച്ചിടുന്ന രീതി)." },
      { word: "Sick",              meaning: "സുഖമില്ലാത്ത." },
      { word: "Sick child",        meaning: "സുഖമില്ലാത്ത കുഞ്ഞ്." }
    ]
  },
  {
    id: 4,
    title: "Paragraph 4",
    englishText: "He took her to the rear of the courtyard. They both looked at the fallen body.",
    sentences: [
      {
        id: "s4-1",
        english: "He took her to the rear of the courtyard.",
        malayalam: "അവൻ അവളെ മുറ്റത്തിന്റെ പിന്നാമ്പുറത്തേക്ക് കൂട്ടിക്കൊണ്ടുപോയി.",
        phrases: [
          { english: "He took her",                       malayalam: "അവൻ അവളെ കൂട്ടിക്കൊണ്ടുപോയി" },
          { english: "to the rear of the courtyard.",     malayalam: "മുറ്റത്തിന്റെ പിന്നാമ്പുറത്തേക്ക്." }
        ]
      },
      {
        id: "s4-2",
        english: "They both looked at the fallen body.",
        malayalam: "അവർ രണ്ടുപേരും ആ വീണുകിടക്കുന്ന ശരീരത്തിലേക്ക് നോക്കി.",
        phrases: [
          { english: "They both looked at",       malayalam: "അവർ രണ്ടുപേരും നോക്കി" },
          { english: "the fallen body.",          malayalam: "ആ വീണുകിടക്കുന്ന ശരീരത്തിലേക്ക്." }
        ]
      }
    ],
    vocabulary: [
      { word: "Rear",                   meaning: "പിന്നാമ്പുറം." },
      { word: "Courtyard",              meaning: "മുറ്റം." },
      { word: "Rear of the courtyard",  meaning: "മുറ്റത്തിന്റെ പിന്നാമ്പുറം." },
      { word: "Fallen body",            meaning: "നിലത്തുവീണു കിടക്കുന്ന ശരീരം." }
    ]
  },
  {
    id: 5,
    title: "Paragraph 5",
    englishText: "The man was dressed like a ragpicker. There were only a few faded hairs left on his bald skull and very few teeth in his mouth. His pitiful condition seemed that of a drenched great-grandfather. This took away any sense of grandeur he might have had. His huge buzzard wings, dirty and half-plucked, were entangled in the mud.",
    sentences: [
      {
        id: "s5-1",
        english: "The man was dressed like a ragpicker.",
        malayalam: "ആ മനുഷ്യൻ ഒരു ആക്രിപെറുക്കുന്നവനെപ്പോലെയാണ് വസ്ത്രം ധരിച്ചിരുന്നത്.",
        phrases: [
          { english: "The man was dressed like a ragpicker.", malayalam: "ആ മനുഷ്യൻ ഒരു ആക്രിപെറുക്കുന്നവനെപ്പോലെയാണ് വസ്ത്രം ധരിച്ചിരുന്നത്." }
        ]
      },
      {
        id: "s5-2",
        english: "There were only a few faded hairs left on his bald skull and very few teeth in his mouth.",
        malayalam: "അയാളുടെ കഷണ്ടി തലയിൽ കുറച്ചു മങ്ങിയ മുടികൾ മാത്രമേ ബാക്കിയുണ്ടായിരുന്നുള്ളൂ, വായിലാകട്ടെ വളരെ കുറച്ചു പല്ലുകളും.",
        phrases: [
          { english: "There were only a few faded hairs left on his bald skull", malayalam: "അയാളുടെ കഷണ്ടി തലയിൽ കുറച്ചു മങ്ങിയ മുടികൾ മാത്രമേ ബാക്കിയുണ്ടായിരുന്നുള്ളൂ," },
          { english: "and very few teeth in his mouth.",                         malayalam: "വായിലാകട്ടെ വളരെ കുറച്ചു പല്ലുകളും." }
        ]
      },
      {
        id: "s5-3",
        english: "His pitiful condition seemed that of a drenched great-grandfather.",
        malayalam: "നനഞ്ഞുകുതിർന്ന ഒരു മുത്തശ്ശന്റേതുപോലെയുള്ള ദയനീയമായ ഒരു അവസ്ഥയായിരുന്നു അയാളുടേത്.",
        phrases: [
          { english: "His pitiful condition",                          malayalam: "അയാളുടെ ദയനീയമായ അവസ്ഥ" },
          { english: "seemed that of a drenched great-grandfather.",   malayalam: "നനഞ്ഞുകുതിർന്ന ഒരു മുത്തശ്ശന്റേതുപോലെ തോന്നിച്ചു." }
        ]
      },
      {
        id: "s5-4",
        english: "This took away any sense of grandeur he might have had.",
        malayalam: "ഇത് അയാൾക്ക് ഉണ്ടായേക്കാമായിരുന്ന എല്ലാവിധ പ്രൗഢിയും ഇല്ലാതാക്കി.",
        phrases: [
          { english: "This took away any sense of grandeur", malayalam: "ഇത് ഏതൊരു പ്രൗഢിബോധവും ഇല്ലാതാക്കി" },
          { english: "he might have had.",                   malayalam: "അയാൾക്ക് ഉണ്ടായേക്കാമായിരുന്ന." }
        ]
      },
      {
        id: "s5-5",
        english: "His huge buzzard wings, dirty and half-plucked, were entangled in the mud.",
        malayalam: "കഴുകന്റേതുപോലെയുള്ള അയാളുടെ വലിയ ചിറകുകൾ, അഴുക്കുപിടിച്ചതും പകുതിയും തൂവലുകൾ കൊഴിഞ്ഞതുമായിരുന്നു, അവ ചെളിയിൽ കുടുങ്ങിക്കിടക്കുകയായിരുന്നു.",
        phrases: [
          { english: "His huge buzzard wings,",       malayalam: "കഴുകന്റേതുപോലെയുള്ള അയാളുടെ വലിയ ചിറകുകൾ," },
          { english: "dirty and half-plucked,",       malayalam: "അഴുക്കുപിടിച്ചതും പകുതിയും തൂവലുകൾ കൊഴിഞ്ഞതുമായ," },
          { english: "were entangled in the mud.",    malayalam: "ചെളിയിൽ കുടുങ്ങിക്കിടക്കുകയായിരുന്നു." }
        ]
      }
    ],
    vocabulary: [
      { word: "Ragpicker",     meaning: "ആക്രിപെറുക്കുന്നയാൾ / അഴുക്കുതുണിപെറുക്കുന്നയാൾ." },
      { word: "Faded hairs",   meaning: "മങ്ങിയ / നിറംകെട്ട മുടികൾ." },
      { word: "Bald skull",    meaning: "കഷണ്ടിത്തല." },
      { word: "Pitiful condition", meaning: "ദയനീയമായ അവസ്ഥ." },
      { word: "Drenched",      meaning: "നനഞ്ഞുകുതിർന്ന." },
      { word: "Great-grandfather", meaning: "മുത്തശ്ശൻ (അപ്പൂപ്പന്റെ അച്ഛൻ)." },
      { word: "Grandeur",      meaning: "പ്രൗഢി / രാജകീയത / വലിപ്പം." },
      { word: "Buzzard wings", meaning: "കഴുകന്റേതുപോലെയുള്ള ചിറകുകൾ." },
      { word: "Half-plucked",  meaning: "പകുതി തൂവലുകൾ കൊഴിഞ്ഞ / പിഴുതെടുക്കപ്പെട്ട." },
      { word: "Entangled",     meaning: "കുടുങ്ങിക്കിടക്കുകയായിരുന്നു." }
    ]
  }
];

// ─────────────────────────────────────────────────────────────────────────────
// GRAMMAR_LECTURE_DATA — updated examples for TO, IN, ON as requested
// ─────────────────────────────────────────────────────────────────────────────
export const GRAMMAR_LECTURE_DATA = {
  prepositions: {
    title: "Prepositions (ഗതികൾ)",
    intro: {
      english: "A Preposition is a word used before a noun or pronoun to show its relationship (like position, direction, or time) with other words in a sentence. Think of it as a 'connector' that tells you where, when, or how something is happening.",
      malayalam: "ഒരു വാക്യത്തിലെ നാമപദങ്ങൾക്കും (Nouns) സർവ്വനാമങ്ങൾക്കും (Pronouns) മുൻപിൽ വന്ന്, അവയ്ക്ക് മറ്റ് വാക്കുകളുമായുള്ള ബന്ധത്തെ (സ്ഥലം, സമയം, ദിശ എന്നിവ) കാണിക്കുന്ന വാക്കുകളെയാണ് Prepositions എന്ന് വിളിക്കുന്നത്. ലളിതമായി പറഞ്ഞാൽ, മലയാളത്തിൽ നമ്മൾ വാക്കുകളുടെ കൂടെ ചേർക്കുന്ന 'ഇൽ', 'മേൽ', 'ലേക്ക്', 'ഉടെ' തുടങ്ങിയ പ്രത്യയങ്ങൾക്ക് (Suffixes) പകരമായി ഇംഗ്ലീഷിൽ ഉപയോഗിക്കുന്ന വാക്കുകളാണിത്."
    },
    items: [
      {
        word: "TO",
        explanation: {
          english: "Used to show direction, movement, or a destination toward a place or person.",
          malayalam: "ഒരു സ്ഥലത്തേക്കോ വ്യക്തിയിലേക്കോ ഉള്ള യാത്ര, നീക്കം, അല്ലെങ്കിൽ ദിശ എന്നിവയെ കാണിക്കാൻ ഉപയോഗിക്കുന്നു. മലയാളത്തിലെ '-ലേക്ക്', '-ക്ക്' എന്ന അർത്ഥത്തിൽ ഇത് വരുന്നു."
        },
        examples: [
          { en: "He is going to school.", ml: "അവൻ സ്കൂളിലേക്ക് പോകുന്നു." },
          { en: "My father took me to the zoo.", ml: "എന്റെ അച്ഛൻ എന്നെ മൃഗശാലയിലേക്ക് കൂട്ടിക്കൊണ്ടുപോയി." }
        ]
      },
      {
        word: "IN",
        explanation: {
          english: "Used to show that something is inside an enclosed space, a container, or a defined boundary (like a room, car, or city). It is also used for months and years.",
          malayalam: "ഒരു അടഞ്ഞ സ്ഥലത്തിന്റെയോ, പാത്രത്തിന്റെയോ, അല്ലെങ്കിൽ ഒരു അതിരിന്റെയോ ഉള്ളിൽ ഒന്നിരിക്കുന്നു എന്ന് കാണിക്കാൻ ഉപയോഗിക്കുന്നു. മലയാളത്തിലെ '-ഇൽ', '-കത്ത്' എന്ന അർത്ഥത്തിൽ ഇത് വരുന്നു."
        },
        examples: [
          { en: "The keys are in my pocket.", ml: "താക്കോലുകൾ എന്റെ പോക്കറ്റിനുള്ളിൽ ഉണ്ട്." },
          { en: "There are many books in the bag.", ml: "ബാഗിനുള്ളിൽ ഒരുപാട് പുസ്തകങ്ങളുണ്ട്." }
        ]
      },
      {
        word: "ON",
        explanation: {
          english: "Used to show that an object is resting on top of a surface. It is also used for specific days and dates (e.g., on Monday).",
          malayalam: "ഒരു വസ്തു മറ്റൊരു പ്രതലത്തിന് മുകളിൽ തൊട്ടുകൊണ്ട് ഇരിക്കുന്നു എന്ന് കാണിക്കാൻ ഉപയോഗിക്കുന്നു. മലയാളത്തിലെ '-മേൽ', '-മുകളിൽ' എന്ന അർത്ഥത്തിലാണ് ഇത് വരുന്നത്."
        },
        examples: [
          { en: "The book is on the table.", ml: "പുസ്തകം മേശയ്ക്ക് മുകളിലാണ്." },
          { en: "He put the stamp on the letter.", ml: "അവൻ കത്തിന് മുകളിൽ സ്റ്റാമ്പ് ഒട്ടിച്ചു." }
        ]
      },
      {
        word: "AT",
        explanation: {
          english: "Used to point out a specific, exact location/point or a specific time (e.g., at 5 PM).",
          malayalam: "ഒരു കൃത്യമായ സ്ഥലത്തെയോ ബിന്ദുവിനെയോ (Exact spot) അല്ലെങ്കിൽ കൃത്യമായ സമയത്തെയോ കാണിക്കാൻ ഉപയോഗിക്കുന്നു. മലയാളത്തിൽ '-ൽ', '-അടുത്ത്' എന്ന അർത്ഥം വരുന്നു."
        },
        examples: [
          { en: "She is standing at the bus stop.", ml: "അവൾ ബസ് സ്റ്റോപ്പിൽ നിൽക്കുന്നു. (കൃത്യമായ സ്ഥലം)" },
          { en: "The class starts at 9 AM.", ml: "ക്ലാസ്സ് 9 മണിക്ക് ആരംഭിക്കുന്നു. (കൃത്യമായ സമയം)" }
        ]
      },
      {
        word: "OF",
        explanation: {
          english: "Used to show belonging, connection, or possession (who owns something or what it belongs to).",
          malayalam: "ഉടമസ്ഥതയെയോ ബന്ധത്തെയോ കാണിക്കാൻ ഉപയോഗിക്കുന്നു. മലയാളത്തിലെ '-ഉടെ' എന്ന പ്രത്യയത്തിന് തുല്യമാണിത്."
        },
        examples: [
          { en: "This is the car of my brother.", ml: "ഇത് എന്റെ സഹോദരന്റെ കാറാണ്." },
          { en: "The color of the sky is blue.", ml: "ആകാശത്തിന്റെ നിറം നീലയാണ്." }
        ]
      },
      {
        word: "WITH",
        explanation: {
          english: "Used to mean together with someone/something, or to show the tool/instrument used to do an action.",
          malayalam: "ഒരാളുടെയോ ഒന്നിന്റെയോ കൂടെ/ഒപ്പം എന്ന് കാണിക്കാനും, അല്ലെങ്കിൽ ഒരു കാര്യം ചെയ്യാൻ നമ്മൾ ഉപയോഗിച്ച ഉപകരണത്തെ സൂചിപ്പിക്കാനും ഇത് ഉപയോഗിക്കുന്നു. മലയാളത്തിൽ '-ഓടൊപ്പം', '-കൂടെ', '-കൊണ്ട്' എന്നീ അർത്ഥങ്ങൾ വരുന്നു."
        },
        examples: [
          { en: "I went to the market with my mother.", ml: "ഞാൻ എന്റെ അമ്മയോടൊപ്പം മാർക്കറ്റിൽ പോയി." },
          { en: "He wrote the letter with a pen.", ml: "അവൻ ഒരു പേന കൊണ്ട് കത്തെഴുതി." }
        ]
      }
    ],
    table: [
      { word: "To",   shortcut: "Towards a destination",      meaning: "ലേക്ക് / ലേയ്ക്ക്" },
      { word: "In",   shortcut: "Inside something",           meaning: "ഉള്ളിൽ / ഇൽ" },
      { word: "On",   shortcut: "On top of a surface",        meaning: "മുകളിൽ / മേൽ" },
      { word: "At",   shortcut: "Exact point / specific time",meaning: "ൽ / അടുത്ത് / ക്ക്" },
      { word: "Of",   shortcut: "Belonging to / Possession",  meaning: "ഉടെ" },
      { word: "With", shortcut: "Along with / Using a tool",  meaning: "കൂടെ / ഒപ്പം / കൊണ്ട്" }
    ]
  },
  articles: {
    title: "Articles (നിർണ്ണായക പദങ്ങൾ)",
    intro: {
      english: "An Article is a word used before a noun to show whether the noun is specific (particular) or general (any common one). There are three articles in English: A, An, and The.",
      malayalam: "ഒരു നാമപദം (Noun) നമ്മൾ പറയുന്നത് കൃത്യമായി ഏതെങ്കിലും ഒന്നിനെക്കുറിച്ചാണോ (Specific) അതോ പൊതുവായി ഏതെങ്കിലും ഒന്നിനെക്കുറിച്ചാണോ (General) എന്ന് വ്യക്തമാക്കാൻ നാമപദങ്ങൾക്ക് മുൻപിൽ ഉപയോഗിക്കുന്ന വാക്കുകളാണ് Articles. ഇംഗ്ലീഷിൽ ആകെ മൂന്ന് ആർട്ടിക്കിളുകളാണ് ഉള്ളത്: A, An, കൂടാതെ The."
    },
    types: [
      { name: "Indefinite Articles (നിശ്ചിതമല്ലാത്തവ)", desc: "A, An (മലയാളത്തിൽ 'ഒരു' എന്ന അർത്ഥം വരുന്നു)." },
      { name: "Definite Article (നിശ്ചിതമായവ)", desc: "The (മലയാളത്തിൽ 'ആ' എന്ന അർത്ഥത്തോട് അടുത്തു നിൽക്കുന്നു)." }
    ],
    items: [
      {
        word: "A (Indefinite Article)",
        explanation: {
          english: "Used before singular, countable nouns that start with a consonant sound (sounds other than a, e, i, o, u). It points to something in general.",
          malayalam: "വ്യഞ്ജനാക്ഷര ശബ്ദങ്ങളിൽ (Consonant sounds) തുടങ്ങുന്നതും, എണ്ണാൻ കഴിയുന്നതുമായ ഒരു വചന (Singular) നാമപദങ്ങൾക്ക് മുൻപിൽ 'ഒരു' എന്ന പൊതുവായ അർത്ഥത്തിൽ ഇത് ഉപയോഗിക്കുന്നു."
        },
        examples: [
          { en: "a book", ml: "ഒരു പുസ്തകം — ഇവിടെ 'book' തുടങ്ങുന്നത് 'b' എന്ന വ്യഞ്ജനാക്ഷര ശബ്ദത്തിലാണ്." },
          { en: "a rag picker", ml: "ഒരു ആക്രിപെറുക്കുന്നവൻ — പൊതുവായി ഒരാളെ താരതമ്യം ചെയ്യാൻ ഉപയോഗിച്ചു." },
          { en: "a university", ml: "വ്യാകരണ രഹസ്യം: ഇവിടെ എഴുതുമ്പോൾ 'u' ആണെങ്കിലും ശബ്ദം വരുന്നത് 'yu' (യൂ) എന്ന വ്യഞ്ജനാക്ഷരത്തിന്റേതാണ്. അതുകൊണ്ട് 'a' ഉപയോഗിക്കുന്നു." }
        ]
      },
      {
        word: "AN (Indefinite Article)",
        explanation: {
          english: "Used before singular, countable nouns that start with a vowel sound (a, e, i, o, u). It also means 'one' in general.",
          malayalam: "സ്വരാക്ഷര ശബ്ദങ്ങളിൽ (Vowel sounds - അ, ഇ, ഉ, എ, ഒ...) തുടങ്ങുന്ന ഒരൊറ്റ വസ്തുവിനെയോ വ്യക്തിയെയോ പൊതുവായി സൂചിപ്പിക്കാൻ 'ഒരു' എന്ന അർത്ഥത്തിൽ ഇത് ഉപയോഗിക്കുന്നു."
        },
        examples: [
          { en: "an apple", ml: "ഒരു ആപ്പിൾ — 'apple' തുടങ്ങുന്നത് 'ആ' എന്ന സ്വരശബ്ദത്തിലാണ്." },
          { en: "an umbrella", ml: "ഒരു കുട — 'umbrella' തുടങ്ങുന്നത് 'അ' എന്ന സ്വരശബ്ദത്തിലാണ്." },
          { en: "an hour", ml: "വ്യാകരണ രഹസ്യം: ഇവിടെ 'h' സൈലന്റ് ആണ്, ശബ്ദം തുടങ്ങുന്നത് 'ഔവർ' (our) എന്ന് സ്വരാക്ഷരത്തിലാണ്. അതുകൊണ്ട് 'an' വരുന്നു." }
        ]
      },
      {
        word: "THE (Definite Article)",
        explanation: {
          english: "Used before both singular and plural nouns when the noun is specific, unique, or has already been mentioned before.",
          malayalam: "നമ്മൾ സംസാരിക്കുന്ന വ്യക്തിക്കോ വസ്തുവിനോ കൃത്യമായ ഒരു വ്യക്തിത്വമുള്ളപ്പോൾ, അല്ലെങ്കിൽ അതിനെക്കുറിച്ച് നമ്മൾ മുൻപ് സംസാരിച്ചിട്ടുള്ളപ്പോൾ (Specific) ആണ് 'The' ഉപയോഗിക്കുന്നത്. മലയാളത്തിൽ 'ആ' (ആ പുസ്തകം, ആ മനുഷ്യൻ) എന്ന് ചൂണ്ടിക്കാണിക്കുന്ന അർത്ഥത്തിലാണ് ഇത് വരുന്നത്. ലോകത്തിൽ ഒന്നേയുള്ളൂ എന്ന് പറയുന്ന യുണീക്ക് ആയ കാര്യങ്ങൾക്കും ഇത് ഉപയോഗിക്കും."
        },
        examples: [
          { en: "They looked at the fallen body.", ml: "അവർ ആ വീണുകിടക്കുന്ന ശരീരത്തിലേക്ക് നോക്കി — അവിടെ കിടക്കുന്ന ആ പ്രത്യേക ശരീരത്തെക്കുറിച്ച് പറയുന്നതിനാൽ 'the' വന്നു." },
          { en: "The Sun rises in the east.", ml: "സൂര്യൻ കിഴക്കുദിക്കുന്നു — സൂര്യൻ ലോകത്തിൽ ഒന്നേയുള്ളൂ (Unique), അതുകൊണ്ട് 'The' ഉപയോഗിച്ചു." },
          { en: "I saw a dog. The dog was barking.", ml: "ഞാൻ ഒരു പട്ടിയെ കണ്ടു. ആ പട്ടി കുരയ്ക്കുകയായിരുന്നു — ആദ്യത്തെ വരിയിൽ പൊതുവായി പറഞ്ഞ പട്ടി (a dog), രണ്ടാമത്തെ വരിയിൽ കൃത്യമായ ഒന്നായി മാറിയപ്പോൾ 'The dog' ആയി." }
        ]
      }
    ],
    table: [
      { article: "A",   rule: "Consonant Sound (വ്യഞ്ജന ശബ്ദം)",       meaning: "ഒരു (പൊതുവായത്)" },
      { article: "An",  rule: "Vowel Sound (സ്വര ശബ്ദം)",              meaning: "ഒരു (പൊതുവായത്)" },
      { article: "The", rule: "Specific / Already Known / Unique",     meaning: "ആ (പ്രത്യേകം എടുത്തു പറയുന്ന ഒന്ന്)" }
    ]
  }
};

export const GRAMMAR_QUIZ_DATA = [
  {
    id: 1,
    question: "He took her .................. the rear of the courtyard.",
    options: ["on", "the", "to", "in"],
    correctAnswer: "to",
    explanation: "ഇവിടെ 'to' ആണ് ശരിയായ വാക്ക്. കാരണം, നമ്മൾ ഒരു സ്ഥലത്തേക്ക് പോകുന്നതിനെയോ, നീങ്ങുന്നതിനെയോ അല്ലെങ്കിൽ ഒരാളെ കൂട്ടിക്കൊണ്ടുപോകുന്നതിനെയോ (Direction/Movement) കാണിക്കാൻ ഇംഗ്ലീഷിൽ എപ്പോഴും 'to' ആണ് ഉപയോഗിക്കാറുള്ളത്. അവൻ അവളെ മുറ്റത്തിന്റെ പിൻഭാഗത്തേക്ക് കൂട്ടിക്കൊണ്ടുപോയി എന്നാണ് ഈ വരിയുടെ അർത്ഥം. അതുകൊണ്ട് ആ 'ലേക്ക്' (മുറ്റത്തിന്റെ പിൻഭാഗത്തേക്ക്) എന്ന അർത്ഥം വരാൻ ഇവിടെ 'to' തന്നെ വരണം."
  },
  {
    id: 2,
    question: "They both looked at ............. fallen body.",
    options: ["a", "the", "an", "to"],
    correctAnswer: "the",
    explanation: "ഇവിടെ 'the' ആണ് ശരിയായ ഉത്തരം. കാരണം, അവർ രണ്ടുപേരും നോക്കുന്നത് അവിടെ വീണുകിടക്കുന്ന കൃത്യമായി അറിയാവുന്ന, ഒരു പ്രത്യേക ശരീരത്തിലേക്കാണ് (Specific fallen body). ഇംഗ്ലീഷിൽ നമ്മൾ നേരത്തെ സംസാരിച്ചതോ അല്ലെങ്കിൽ നമുക്ക് കൃത്യമായി ഏതെന്നറിയാവുന്നതോ ആയ ഒരു നിശ്ചിത കാര്യത്തെക്കുറിച്ച് (Specific noun) പറയുമ്പോഴാണ് Definite Article ആയ 'the' ഉപയോഗിക്കുന്നത്. ഇവിടെ ആ മുറ്റത്ത് ഒരു മനുഷ്യൻ വീണുകിടപ്പുണ്ട് എന്ന് അവർക്ക് മുൻപേ അറിയാം, ആ ശരീരത്തെക്കുറിച്ചാണ് അവർ വീണ്ടും പറയുന്നത്."
  },
  {
    id: 3,
    question: "The man was dressed like ............. rag picker.",
    options: ["of", "the", "an", "a"],
    correctAnswer: "a",
    explanation: "ഇവിടെ 'a' ആണ് ശരിയായ ഉത്തരം. കാരണം, ഒരു വ്യക്തിയോ വസ്തുവോ ഏത് തരത്തിൽ പെട്ടതാണെന്ന് പൊതുവായി വ്യക്തമാക്കുമ്പോഴാണ് നമ്മൾ Indefinite Article ആയ 'a' ഉപയോഗിക്കുന്നത്. അയാൾ ഒരു ആക്രിപെറുക്കുന്നവനെപ്പോലെ (rag picker) വസ്ത്രം ധരിച്ചിരുന്നു എന്നാണ് ഈ വരിയുടെ അർത്ഥം. ഇവിടെ 'rag picker' എന്നത് ഒരു പൊതുവായ തൊഴിലിനെയോ ഗ്രൂപ്പിനെയോ ആണ് സൂചിപ്പിക്കുന്നത്, അല്ലാതെ നമുക്ക് മുൻപേ പരിചയമുള്ള ഒരു പ്രത്യേക വ്യക്തിയെക്കുറിച്ചല്ല പറയുന്നത്. സിംഗുലർ ആയ (ഒരു വചനം) നാമപദങ്ങൾക്ക് മുൻപിൽ 'ഒരു' എന്ന അർത്ഥം വരാൻ നമ്മൾ 'a' ഉപയോഗിക്കുന്നു."
  },
  {
    id: 4,
    question: "There were only a few faded hairs left .............. his bald skull.",
    options: ["on", "in", "at", "of"],
    correctAnswer: "on",
    explanation: "ഇവിടെ 'on' ആണ് ശരിയായ ഉത്തരം. കാരണം, ഒരു പ്രതലത്തിന് മുകളിലോ (surface) ശരീരഭാഗത്തിന് മുകളിലോ ഒരു വസ്തു സ്ഥിതി ചെയ്യുന്നു എന്ന് കാണിക്കാൻ നമ്മൾ ഇംഗ്ലീഷിൽ 'on' ആണ് ഉപയോഗിക്കുന്നത്. അയാളുടെ കഷണ്ടി തലയ്ക്ക് (bald skull) മുകളിൽ ഏതാനും ചില നരച്ച രോമങ്ങൾ മാത്രമാണ് ശേഷിച്ചിരുന്നത് എന്നാണ് ഈ വരിയുടെ അർത്ഥം. തലയുടെ പ്രതലത്തിന് മുകളിലാണ് മുടി ഉണ്ടാകുക എന്നതുകൊണ്ട് ഇവിടെ 'on' തന്നെ വരണം."
  },
  {
    id: 5,
    question: "And very few teeth ............ his mouth.",
    options: ["on", "with", "in", "of"],
    correctAnswer: "in",
    explanation: "ഇവിടെ 'in' ആണ് ശരിയായ ഉത്തരം. കാരണം, ഒരു അടഞ്ഞ സ്ഥലത്തിന്റെയോ അല്ലെങ്കിൽ ഒരു വസ്തുവിന്റെയോ ഉള്ളിൽ (inside an enclosed space) ഒന്നിലധികം കാര്യങ്ങൾ സ്ഥിതി ചെയ്യുന്നു എന്ന് കാണിക്കാൻ നമ്മൾ ഇംഗ്ലീഷിൽ എപ്പോഴും 'in' ആണ് ഉപയോഗിക്കുന്നത്. അയാളുടെ വായയ്ക്കുള്ളിൽ (mouth) വളരെ കുറച്ചു പല്ലുകൾ മാത്രമേ ഉണ്ടായിരുന്നുള്ളൂ എന്നാണ് ഈ വരിയുടെ അർത്ഥം. വായ എന്നത് ഉള്ളിലേക്ക് കുഴിഞ്ഞ ഒരു ഭാഗമായതുകൊണ്ട് അതിനകത്തുള്ള പല്ലുകളെ സൂചിപ്പിക്കാൻ 'in' തന്നെ ഉപയോഗിക്കണം."
  }
];
